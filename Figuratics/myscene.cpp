...
MyScene::MyScene(QObject *parent) : QGraphicsScene(parent) { myText = nullptr; }

MyScene::~MyScene() {}

//---------------------------------------------------FIGURE_TYPE

void MyScene::setFigureType(int figureType) { figureType_ = figureType; }
  ...
  if (event->buttons() & !Qt::LeftButton) {
    ...
    switch (figureType_) {
      ...
      case Text:
        ...
        connect(myText, &MyTextClass::lostFocus, this,
                &MyScene::editorLostFocus);

        connect(myText, &MyTextClass::selectedChange, this,
                &MyScene::itemSelected);
      ...
      case SelectItem:
        if (!items().isEmpty()) {
          ...
          figure = items(event->scenePos()).first();
          qDebug() << items(event->scenePos()).first()->type();
...
void MyScene::mouseReleaseEvent(QGraphicsSceneMouseEvent *event) {
  ...
  switch (figureType_) {
    case Pencil:
      savePancil.push_back(&myGroup);
      break;
    case Text:
      event->ignore(); /*saveContainer_.push_back(myText); */
      qDebug() << "Text Item Release Ignore";
      break;
    case MoveItem:
      figure->setSelected(false);
      event->ignore();
      qDebug() << "Move Item Release Ignore";
      break;
    ...
    switch (figureType_) {
      ...
      case MoveItem: {
        if (saveContainer_.isEmpty()) {
          QWidget *my = nullptr;
          QMessageBox::information(my, (""), tr("No OBJECT to move"),
                                   QMessageBox::Ok);
...
void MyScene::drawLine() {
  ...
  figure = addLine(start_point.x(), start_point.y(), end_point.x(),
...
void MyScene::drawTriangle() {
  ...
  triangle << QPointF(start_point.x() +
                          (end_point.x() > start_point.x() ? +1 : -1) *
                              abs((end_point.x() - start_point.x()) / 2),
                      start_point.y())
           << QPointF((end_point.x() > start_point.x()) ? end_point.x()
                                                        : start_point.x(),
                      end_point.y())
           << QPointF((end_point.x() > start_point.x()) ? start_point.x()
                                                        : end_point.x(),
                      end_point.y());
...
void MyScene::drawCircle() {
  ...
  figure = addEllipse(start_point.x(), start_point.y(), w, h, QPen(myPen),
                      QBrush(myBrush));
  figure->setRotation(myRotate);
  figure->setOpacity(myOpacity);
  figure->setZValue(myStrartZValue);
...
void MyScene::drawPoints() {
  ...
  figure = addEllipse(end_point.x(), end_point.y(), 4, 4, QPen(Qt::black),
                      QBrush(myBrush));
  figure->setOpacity(myOpacity);
  figure->setFlag(QGraphicsItem::ItemIsSelectable, true);
}
...
void MyScene::colourDial(QColor colour) { myPen.setColor(colour); }

//---------------------------------------------------LINE_WIDTH

void MyScene::setLineWidth(int lineWidth) { myPen.setWidth(lineWidth); }
    ...
    if (!selectedItems().isEmpty()) {
      ...
      if (items().at(index)->type() == MyTextClass::Type) {
        ...
        QGraphicsTextItem *temp =
            qgraphicsitem_cast<MyTextClass *>(this->items().at(index));
        temp->setDefaultTextColor(colourFill);
      }

      if (items().at(index)->type() == 4) {
        QGraphicsEllipseItem *temp1 =
            qgraphicsitem_cast<QGraphicsEllipseItem *>(this->items().at(index));
        temp1->setBrush(QBrush(colourFill));
      }

      if (items().at(index)->type() == 3) {
        QGraphicsRectItem *temp1 =
            qgraphicsitem_cast<QGraphicsRectItem *>(this->items().at(index));
        temp1->setBrush(QBrush(colourFill));
      }

      if (items().at(index)->type() == 5) {
        QGraphicsPolygonItem *temp1 =
            qgraphicsitem_cast<QGraphicsPolygonItem *>(this->items().at(index));
        temp1->setBrush(QBrush(colourFill));
      }
    }
  }
...
void MyScene::bringToFront() {
  if (!items().isEmpty() & !selectedItems().isEmpty()) {
    ...
    figure->setZValue(figure->zValue() + 1);
    qDebug() << figure->zValue();
    emit signalListWidget();
  }
}
...
void MyScene::scaleFigure(int scale) { figure->setScale(scale); }
